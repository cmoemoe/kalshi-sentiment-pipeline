...
def fetch_market(ticker):
    try:
        response = requests.get(f'{BASE_URL}/markets/{ticker}', timeout=10)
...
def write_prices():
    ...
    cursor.close()
    conn.close()
    print(f"Wrote {written} rows to Snowflake at {time.strftime('%Y-%m-%d %H:%M:%S')}")

while True:
    try:
        write_prices()
    except Exception as e:
        print(f"Write cycle error: {e}")
